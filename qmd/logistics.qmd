...
-   Packages
		-   {planr} - uses opening inventory, sales forecasts and supply variables to calculate projected inventory and projected coverage calculations ([article](https://www.r-bloggers.com/2022/10/using-r-in-inventory-management-and-demand-forecasting/))
...
-   Order Components![](./_resources/Logistics.resources/image.8.png)
...
-   Metrics for judging demand forecasts![](./_resources/Logistics.resources/1-ni6N2L4yp0ynlOzL01f8Fg.png)
...
-   Notes from [Are Your Demand Forecasts Hurting Profits and Service Levels](https://towardsdatascience.com/are-your-demand-forecasts-hurting-profits-and-service-levels-f61af7c5ff)
		-   Blue Dot Thinking’s [WasteNot API service](http://www.bluedotthinking.com/) ($)
			WasteNot (logistics analytics firm) uses prophet for demand forecasting which consistently under-predicts, so they use an adjustment for better forecasts
...
-   Forecasting shocks is difficult for an algorithm
		...
		-   Approaches for manually replacing oulier values
				...
				-   Replace with one of the previous methods, then apply a smoothing algorithm![](./_resources/Logistics.resources/image4-1.png)
...
-   Intermittent Demand
		...
		-   Diagnostics
		-   Do NOT use metrics based on absolute errors (e.g MASE, MAPE) _by themselves_
		...
		-   Accuracy Implication Metrics - measure the effect of the forecast on inventory![](./_resources/Logistics.resources/Screenshot-(1319).png)
				-   Y Axis: 1 - Fill\_Rate; X Axis: Average on-hand inventory
...
-   **Fill rate** (aka **Fulfillment Rate**)- the percentage of orders that you can ship from your available stock without any lost sales, backorders, or stockouts.![](./_resources/Logistics.resources/Screenshot-(494).png)
...
-   Reasons for safety stock:
	...
	**Stockouts** - out of stock events
		-   Safety stock determinations are not intended to eliminate all stockouts—just the majority of them
		Usually caused by:
...
-   **(Cycle) Service level (Z)** \- the probability that the amount of inventory on hand during the lead time is sufficient to meet expected demand – that is, the probability that a stockout will not occur.
		...
		-   Example: ![](./_resources/Logistics.resources/Screenshot-(493).png)
...
-   Safety Stock equations
		...
		-   demand and lead time _variability_ are independent
				-   **Safety stock = Z ×** **√\[(PC/T1  ×  σD2 ) + (σLT × Davg)2\]**
		-   demand and lead time _variability_ are not independent
...
-   Costs of holding stock:
		...
		-   storage
		-   depreciation
-   ![](./_resources/Logistics.resources/ql_e515353171060899a1dc73b3daa97572_l3.png)

Order-Up-To (OUT) Level Policy
![](./_resources/Logistics.resources/Screenshot-(1316).png)
...
-   Other methods for determining forecast performance have serious flaws
		...
		-   FVA: The change in a forecasting accuracy metric that can be attributed to a particular process or participant in the forecasting process.
				-   Example![](./_resources/Logistics.resources/Screenshot-(807).png)
...
-   Forecast Accuracy (FA) metrics are difficult to understand for normals
		...
		-   _“The forecast is always wrong!”, “The forecast is too this”, “The forecast is not enough that”, “What does 70% FA mean?”, “Is this good or bad?”_
...
_π_(_q__t_,_y__t_)\={_p__y__t_−_v__q__t_−_c__h_(_q__t_−_y__t_),_p__q__t_−_v__q__t_−_c__s_(_y__t_−_q__t_),for ​_q__t_≥_y__t_for ​_q__t_<_y__t_,
...
-   Forecast Supply and Demand
		...
		-   Counterfactual to understand how to make tradeoffs between supply and costs
				...
				-   I assume "incentive\_level" is a variable in the supply forecast model, so this could just be adjusted in "newdata" and a  prediction (or maybe during a training session?) made to see the effects on supply.
-   Decision Making
...
-   Pareto Plot
		...
		-   "BOX" is the number of box/packs picked of that product ("SKU") for that order ("ORDER\_NUMBER") on that date ("DATE\_FORMAT")
		-   Preprocessing
