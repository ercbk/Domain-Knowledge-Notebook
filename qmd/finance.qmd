# Finance {#sec-finance .unnumbered}

## Misc {#sec-finance-misc .unnumbered}
...
-   Misleading chart ([Thread](https://twitter.com/choffstein/status/1640034865817985024))\
    ![](./_resources/Finance.resources/image.png){width="448"}\
...
-   Workflow
    ...
    ![From Machine Learning for Algorithmic Trading](_resources/Finance.resources/ml4t_workflow-1.png){width="532"}

## Terms {#sec-finance-terms .unnumbered}
...
-   [**Discount Rate**]{style="color: #009499"} - Represents a minimum rate of return acceptable to the investor, or an assumed rate of inflation.
...
-   [**False Strategy Theorem**]{style="color: #009499"} - Gives the threshold for which a Sharpe Ratio would be significant.
...
-   [**Holding Period Return (HPR)**]{style="color: #009499"} - The total return (%) received from holding an asset or portfolio of assets over a period of time.
    ...
    $$
    \text{HPR} = \frac{(\operatorname{End\_Value} - \operatorname{Initial\_Value}) + \operatorname{Income}}{\operatorname{Initial\_Value}}
    $$
...
-   [**Internal Rate of Return**]{style="color: #009499"} - The rate of return using discounted cash flows. The rate of return necessary for a project to break even. The internal rate of return is a discount rate that makes the net present value (NPV) of all cash flows from a particular project or investment equal to zero. In general, projects with higher IRRs are more favorable than projects with lower IRRs, as the expected rate of return on these projects is greater. (Also see MIRR, XIRR)

    $$
    NPV = -C_0 + \sum_{t=1}^T \frac{C_t}{(1-r)^t} = 0
    $$
    ...
    -   Variables
        ...
        -   $C_0$: Initial investment (aka a cash outlay) (notice the negative sign)
    ...
    -   Disadvantages
        ...
        -   Assumes cash flows will be reinvested into projects with same IRR for each compounding year of the project. Business enviroments change, so it's not reasonable to assume investment opportuniities with the samee IRR will be available every year.
...
-   [**Modified Internal Rate of Return (MIRR)**]{style="color: #009499"} - Replace IRR with MIRR in NPV formula (See Interest Rate of Return for NPV formulat) 

    $$
    \text{MIRR} = \left(\frac{\text{FV}}{-\text{PV}} \right)^{\LARGE{\frac{1}{n}}}
    $$

    -   $n$: The number of periods (where cashflows occur)
    -   $PV$: The Present Value (at the beginning of the first period). The value of an expected cash determined as of the date of valuation. This is negative in the formula, so I assume this means the initial investment.
    
        $$
        \text{PV}_0 = \frac{C_0}{(1 + i)^0} = C_0
        $$
        
    -   $FV$: The Future Value (at the end of the last period). The value of the cash flow at a specified date in the future, based on the value of that asset in the present. See Time Value of Money for the formula.
...
## 60/40 Portfolio Strategy {#sec-finance-6040 .unnumbered}
...
## Momentum Tactical Asset Allocation Strategy {#sec-finance--momassallo .unnumbered}
...
## Volatility Targeting Portfolio {#sec-finance-vtp .unnumbered}
...
## Mean Reversion Strategy or Pairs Trading {#sec-finance-meanrev .unnumbered}
    ...
    -   Resources
        -   [NerdWallet Compound Interest Calculator](https://www.nerdwallet.com/calculator/compound-interest-calculator?trk=nw_gn_5.0)
            -   [Example]{.ribbon-highlight}: if you put \$5,000 in a (5yr) bond with a 4% yield, assuming you reinvest your interest payments, you will have \$6083 by the time it matures.\
                ![](_resources/Finance.resources/nerdwallet-compound-interest-1.png){width="432"}
    -   Notes from
