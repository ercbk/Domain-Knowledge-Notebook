...
## Misc {#sec-finance-val-misc .unnumbered}
    ...
    -   Recommended
        -   Rates of Return: AIRR,MIRR \> IRR
            -   Comparison
                ...
                | Feature                  | AIRR                                    | MIRR                                    |
                |--------------------------|-----------------------------------------|-----------------------------------------|
                | Calculation method       | Average return on each invested dollar  | IRR of the terminal value of cash flows |
                | Multiple solutions       | No                                      | Possible                                |
                | Compatibility with NPV   | Consistent                              | May not be                              |
                | Reinvestment rate        | Assumes constant rate                   | Explicitly factors in                   |
                | Ease of understanding    | Simple and intuitive                    | Complex and less intuitive              |
                | Computational complexity | Can be expensive for complex cash flows | Less computationally intensive          |
...
### Rate of Return (RoR) {#sec-finance-val-ror-ror .unnumbered}
...
-   Types
    -   [Arithmetic]{.underline}: $\sum_{i=1}^T r_i$ where $r_i$ is the return for the time period, $i$.
    -   [Geometric]{.underline} (aka [Compound Annual Growth Rate]{.underline}): $[(1+r_1) \times (1+r_2) \times \cdots \times (1+r_T)]^{1/T} - 1$
        -   Includes compounding effect.
        -   At most is the Arithmetic average but usually less
    -   [Annual Percentage Rate (APR)]{.underline}: $m \times k_m$ where $m$ is the number of compounding periods in a year and $k_m$ is the periodic rate
    ...
    -   [Effective Annual Rate (EAR)]{.underline}: $(1+k_m)^m - 1$
...
### Internal Rate of Return (IRR) {#sec-finance-val-ror-irr .unnumbered}
...
-   Disadvantages
    ...
    -   Assumes cash flows will be reinvested into projects with same IRR for each compounding year of the project. Business enviroments change, so it's not reasonable to assume investment opportuniities with the samee IRR will be available every year. This leads to a more optimistic projection than there should be. ([Video](https://www.youtube.com/watch?v=f0IJegvYPKk))
        ...
        -   Using MIRR instead of IRR solves this issue.\
            ![](_resources/Finance.resources/irr-mirr-change-1.png){.lightbox width="432"}
...
### Modified Internal Rate of Return (MIRR) {#sec-finance-val-ror-mirr .unnumbered}
...
-   Formula
    ...
    -   Variables
        ...
        -   $\text{PV}$: The [absolute]{.underline} sum of the present values of each cash outflow (i.e. negative cash flow), aka Cash Outlay (e.g. costs, investments). Each cash outlay is projected (i.e. discounted) back to year 0 value using the company's borrowing rate. (See IRR for the formula, NPV is the sum of PVs)
...
-   [Example]{.ribbon-highlight}: 7 year Project ([link](https://www.youtube.com/watch?v=Z-CUSIGLk2U&t=342s))
    ...
    | Cash Inflow    |          |         |        | 6000.00 | 7500.00  | 1250.00 | 8010.00 | 9000.00  |
...
### Average Internal Rate of Return (AIRR) {#sec-finance-val-ror-airr .unnumbered}
...
-   Notes from Videos: [AIRR: A Comprehensive Approach to Rate of Return and Investment Decisions](https://www.youtube.com/playlist?list=PL66j5y1ZGZ9FpUbgouPUgiZDFEONAJ1N8)
...
-   [Formula Using Net Present Value and Cash Flows]{.underline}\
    $$
...
-   [Formula Using Time-Varying Cost of Capital]{.underline}\
    $$
    \begin{align}
...
-   [Example]{.ribbon-highlight}: Net Present Value and Cash Flows Formula for AIRR
...
-   Also see [Forcasting, Statistical \>\> Interval Forecasting](https://ercbk.github.io/Data-Science-Notebook/qmd/forecasting-statistical.html#sec-fcast-stat-intv){style="color: green"}
        ...
        -   For private companies, the beta from a peer publicly traded company or the average beta of a group of peer publicly traded companies can be used. Private companies may have differenct capital structures (see WACC weights), so each peer company's beta from the peer group of companies needs be *unlevered* (i.e. remove influence of leverage) to obtain their **Asset Betas**.
            ...
            -   Finally the peer group's asset beta is relevered using the company's target capital structure
                ...
                -   The company's *target* capital structure is oftentimes its *current* capital structure\
                    ![](_resources/Finance.resources/opt-cap-struct-1.png){.lightbox width="432"}
...
-   [(Advanced) Capital Asset Pricing Model (CAPM)]{.underline}
    ...
    -   $R_z$: Specific Company Risk Premium (SCRP) (Reward for bearing unsystematic risk)
        ...
        -   Highland Global method for estimating SCRP
            ...
            ![](_resources/Finance.resources/specific-co-risk-prem-1.png){.lightbox width="532"}
...
-   Methods\
    ![](_resources/Finance.resources/cost-of-debt-methods-1.png){.lightbox width="532"}
...
-   [Project Necessary Variables]{.underline}
    ...
    -   [Example]{.ribbon-highlight}: Percentage of Sales Method for COGS, Gross Margin, and SG&A
        -   Get historical data for Sales (aka Revenue), COGS, Gross Margin, SG&A, EBITDA, D&A, and EBIT for the last 3 to 5 years. Get a consensus of projected estimates for Sales, EBITDA, and EBIT.\
            ![](_resources/Finance.resources/dcf-varproj-1.png){.lightbox width="532"}
        ...
        -   Transform variable amounts to a percentage of sales for each year\
            ![](_resources/Finance.resources/dcf-varproj-perc-1.png){.lightbox width="532"}
        -   For **COGS**, **Gross Margin**, and **SG&A**, calculate the average percentage (last column). Then, multiply each variable's average percentage times each projected Sales estimate to get each variable projected estimate for each period of the horizon.\
            ![](_resources/Finance.resources/dcf-varproj-fillin-1.png){.lightbox width="532"}
...
-   [Determining the Terminal Value]{.underline}
    ...
    -   PGM with correction
        ...
        -   Follows Life Cycle theory that the rate of return on capital will eventually equal the cost of capital due competitors entering the market. It more closely represents the reality that net profit on these cash flows will eventually be zero.
            ...
            ![](_resources/Finance.resources/life-cyc-1.png){.lightbox width="332"}
...
-   [Perform Sensitivity Analysis]{.underline}
    ...
    -   Make sure to include variables that are directly related to business drivers such as sales, operating costs, and investment (e.g. Gross Margin, CAPEX, and other financial statment line items). These are the variables that stakeholders will want to see when they look at your analysis. They want to know what levers they can pull to increase the valuation of their business.\
        ![](_resources/Finance.resources/sens-biz-drivers-1.png){.lightbox width="325"}
    ...
    -   [Example]{.ribbon-highlight}: Trying Different WACC and Exit Multiple Values\
        ![](_resources/Finance.resources/sens-ex-1.png){.lightbox width="432"}
    ...
    -   [Example]{.ribbon-highlight}: Trying different Terminal Value Growth Rates and Gross Margins\
        ![](_resources/Finance.resources/sens-ex-2.png){.lightbox width="502"}
