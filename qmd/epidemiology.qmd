...
-   Packages\
    ...
    -   [{]{style="color: #990000"}[GHRmodel](https://bsc-es.github.io/GHRtools/docs/GHRmodel/GHRmodel){style="color: #990000"}[}]{style="color: #990000"} - Supports modeling health outcomes using Bayesian hierarchical spatio-temporal models with complex covariate effects (e.g., linear, non-linear, interactions, distributed lag linear and non-linear models) in the R-INLA framework.
        -   Designed to help users identify key drivers and predictors of disease risk by enabling streamlined model exploration, comparison, and visualization of complex covariate effects.
    ...
    -   [{]{style="color: #990000"}[sparsesurv](https://cran.r-project.org/web/packages/sparsesurv/index.html){style="color: #990000"}[}]{style="color: #990000"} - Provides functions for fitting, forecasting, and prospective outbreak detection in sparse surveillance count time series
        -   Forecasting using future climatic/environmental covariates you supply (e.g., weather or seasonality drivers).
        -   Separates endemic and epidemic states and returns, for each time point, the probability of being in an epidemic state, enabling transparent flagging of emerging outbreaks.
